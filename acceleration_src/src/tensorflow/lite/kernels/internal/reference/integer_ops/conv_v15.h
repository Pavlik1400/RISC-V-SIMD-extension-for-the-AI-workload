...
inline void ConvPerChannel_aligned_4(const ConvParams& params,
  ...
  cfu_op0(CFU_WRITE_ASYNC_WRITE, 0, 1);
  ...
  for (int out_channel = 0; out_channel < output_depth; ++out_channel) {
    ...
    for (int filter_x = 0; filter_x < 8; ++filter_x) {
      for (int in_channel = 0; in_channel < input_depth; in_channel += write_at_once) {
        ...
        // input_cur x is not beyond buffer (due to padding)
...
inline void ConvPerChannel_aligned_2(const ConvParams& params,
  ...
  cfu_op0(CFU_WRITE_ASYNC_WRITE, 0, 0);

  const int32_t buffer_addr_multiplier = sum_at_once / 2;
...
inline void ConvPerChannel(const ConvParams& params,
  ...
  const int input_depth = filter_shape.Dims(3);
  if (input_depth % 4 == 0) {
    return ConvPerChannel_aligned_4(params, output_multiplier, output_shift, input_shape,
                                    input_data, filter_shape, filter_data, bias_shape, bias_data,
                                    output_shape, output_data);
  }
  if (input_depth % 2 == 0) {
    return ConvPerChannel_aligned_2(params, output_multiplier, output_shift, input_shape,
                                    input_data, filter_shape, filter_data, bias_shape, bias_data,
                                    output_shape, output_data);
  } else {
